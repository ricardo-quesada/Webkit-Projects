...
#include "DeviceMotionClientIPhone.h"
#include "DeviceMotionController.h"
#include "DeviceOrientationClient.h"
#include "DeviceOrientationClientIPhone.h"
...
Document::Document(Frame* frame, const KURL& url, bool isXHTML, bool isHTML)
   ...
   /* , m_deviceMotionClient(DeviceMotionClientIPhone::create())
    , m_deviceMotionController(DeviceMotionController::create(m_deviceMotionClient.get()))
    , m_deviceOrientationClient(DeviceOrientationClientIPhone::create())
    ...
    , m_isTelephoneNumberParsingAllowed(true)
{
    ...
    // If this frame has timers paused, we need to suspend the event queue timer as well.
...
void Document::recalcStyle(StyleChange change)
{
    ...
    if (change == Force) {
        ...
        if (Settings* settings = this->settings()) {
            if (settings->fontFallbackPrefersPictographs() && m_styleSelector)
...
void Document::updateStyleIfNeeded()
{
    ...
    static bool isMainThread() { return true; } //Ricardo: lo agrego

void Document::updateStyleForAllDocuments()
{
    //ASSERT(isMainThread() || pthread_main_np()); //Ricardo: comento para seguir
    if (!documentsThatNeedStyleRecalc)
        return;
...
void Document::nodeWillBeRemoved(Node* n)
{
    ...
    removeTouchEventListener(n, true);
}
...
PassRefPtr<Event> Document::createEvent(const String& eventType, ExceptionCode& ec)
{
    ...
    else if (eventType == "GestureEvent")
        event = GestureEvent::create();
#if ENABLE(DEVICE_ORIENTATION)
...
static PassRefPtr<RenderStyle> cloneRenderStyleWithState(const RenderStyle* currentStyle)
{
    ...
    RenderStyle* pseudoStyle = currentStyle->getCachedPseudoStyle(VISITED_LINK);
    if (pseudoStyle)
        newStyle->addCachedPseudoStyle(RenderStyle::clone(pseudoStyle));
