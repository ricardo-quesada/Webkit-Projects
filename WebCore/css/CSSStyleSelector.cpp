...
void CSSStyleSelector::adjustRenderStyle(RenderStyle* style, RenderStyle* parentStyle, Element *e)
{
    ...
    if (style->hasAutoZIndex() && ((e && e->document()->documentElement() == e) || style->opacity() < 1.0f || 
        ...
        // Have fixed position elements create stacking context <rdar://problem/9813262>
...
bool CSSStyleSelector::SelectorChecker::checkOneSelector(CSSSelector* sel, Element* e, HashSet<AtomicStringImpl*>* selectorAttrs, PseudoId& dynamicPseudo, bool isSubSelector, bool encounteredLink, RenderStyle* elementStyle, RenderStyle* elementParentStyle) const
{
    ...
    if (sel->hasAttribute()) {
        ...
        // Same check as above, with the additional check for blockquotes
        ...
        if (elementStyle && (!e->isStyledElement() || (e->hasTagName(blockquoteTag) && attr == typeAttr) || (!static_cast<StyledElement*>(e)->isMappedAttribute(attr) && attr != typeAttr && attr != readonlyAttr))) {
...
// uri || inherit
    ...
    case CSSPropertyWordSpacing:
    {
        ...
        if (primitiveValue && primitiveValue->getIdent() == CSSValueNormal) {
            ...
            if (!primitiveValue)
                return;
            width = primitiveValue->computeLengthFloat(style(), m_rootElementStyle, useSVGZoomRules(m_element) ? 1.0f : zoomFactor);
        }
        switch (id) {
        case CSSPropertyLetterSpacing:
...
// length, percent, number
    case CSSPropertyLineHeight:
    {
        ...
        HANDLE_INHERIT_AND_INITIAL(specifiedLineHeight, SpecifiedLineHeight)
        if (!primitiveValue)
            return;
        Length lineHeight;
        int type = primitiveValue->primitiveType();
...
// shorthand properties
    ...
    case CSSPropertyWebkitImageLoadingBorder: {
        HANDLE_INHERIT_AND_INITIAL(imageLoadingBorder, ImageLoadingBorder)
        if (!primitiveValue) break;
        int id = primitiveValue->getIdent();
        EImageLoadingBorder imageLoadingBorder;
        if (id == CSSValueNone)
            imageLoadingBorder = IMAGE_LOADING_BORDER_NONE;
        else
            imageLoadingBorder = IMAGE_LOADING_BORDER_OUTLINE;
        m_style->setImageLoadingBorder(imageLoadingBorder);
        return;
    }
    ...
    case CSSPropertyWebkitTouchCallout: {
        ...
        m_style->setTouchCalloutEnabled(primitiveValue->getStringValue().lower() != "none");
        return;
    }
    case CSSPropertyWebkitCompositionFillColor: {
        ...
        m_style->setCompositionFillColor(col);
        return;
    }
    case CSSPropertyWebkitCompositionFrameColor: {
        HANDLE_INHERIT_AND_INITIAL(compositionFrameColor, CompositionFrameColor);
        if (!primitiveValue)
            break;
